...
//	class const_iterator;
        ...
        public:
            ...
            iterator(deque *_deque_ptr, int _block_th, int _node_th, int _endnote = 0) : deque_ptr(_deque_ptr),
                                                                                         block_th(_block_th),
                                                                                         node_th(_node_th),
                                                                                         endnote(_endnote) {
            };
...
/*	class const_iterator {
         ...
         */
        iterator end() {
            return iterator(this, last_block_address, blocks[last_block_address].num - 1, 1);
        }
...
//                bl.nodes[insert_pos] = insert_node;
        ...
        iterator erase(iterator pos) {
            ...
            if (bl.next != -1) {//merge
                ...
                if (bl.num + bl_next.num <= Nmax) {
                    if (bl_next.next == -1)
                        last_block_address = bl.next;//FIXME end系列的判定和charu（必须改成pushbsack）也都有问题。还有关于redundancy的计算（endnote）
        ...
        void push_back(const T &value) {
            ...
            ++num;
            if (bl.num < Nmax) {
                bl.nodes.push_back(new T(value));
                bl.num++;
                return;
            }
            Block bl_end;
            bl_end.nodes.push_back(new T(value));
            bl_end.num = 1;
            bl_end.next = -1;
